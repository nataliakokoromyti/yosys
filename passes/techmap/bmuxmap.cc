	...
	void execute(std::vector<std::string> args, RTLIL::Design *design) override
	{
		...
		for (auto cell : module->selected_cells())
		{
			...
			if(pmux_mode)
			{
				...
				SigSpec new_s = module->addWire(NEW_ID2_SUFFIX("sel"), num_cases);
				SigSpec new_data = module->addWire(NEW_ID2_SUFFIX("data"), width);
				for (int val = 0; val < num_cases; val++)
				{
					module->addEq(NEW_ID2_SUFFIX("eq"), sel, SigSpec(val, GetSize(sel)), new_s[val]);
				}
				RTLIL::Cell *pmux = module->addPmux(NEW_ID2_SUFFIX("pmux"), new_a, data, new_s, new_data);
