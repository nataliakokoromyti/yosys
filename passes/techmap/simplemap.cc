...
void simplemap_eqne(RTLIL::Module *module, RTLIL::Cell *cell)
{
	...
	RTLIL::SigSpec xor_out = module->addWire(NEW_ID2_SUFFIX("xor"), max(GetSize(sig_a), GetSize(sig_b)));
	RTLIL::Cell *xor_cell = module->addXor(NEW_ID2, sig_a, sig_b, xor_out, is_signed);
	transfer_src(xor_cell, cell);
	simplemap_bitop(module, xor_cell);
	module->remove(xor_cell);

	RTLIL::SigSpec reduce_out = is_ne ? sig_y : module->addWire(NEW_ID2_SUFFIX("reduce_out"));
...
void simplemap_bmux(RTLIL::Module *module, RTLIL::Cell *cell)
{
	...
	for (int idx = 0; idx < GetSize(sel); idx++) {
		SigSpec new_data = module->addWire(NEW_ID2_SUFFIX("data"), GetSize(data)/2);
...
void simplemap_lut(RTLIL::Module *module, RTLIL::Cell *cell)
{
	...
	for (int idx = 0; GetSize(lut_data) > 1; idx++) {
		SigSpec new_lut_data = module->addWire(NEW_ID2_SUFFIX("data"), GetSize(lut_data)/2);
