...
void simplemap_bitop(RTLIL::Module *module, RTLIL::Cell *cell)
{
	...
	for (int i = 0; i < GetSize(sig_y); i++) {
		RTLIL::Cell *gate = module->addCell(NEW_ID2, gate_type);
		transfer_src(gate, cell);
		gate->setPort(ID::A, sig_a[i]);
...
void simplemap_reduce(RTLIL::Module *module, RTLIL::Cell *cell)
{
	...
	while (sig_a.size() > 1)
	{
		RTLIL::SigSpec sig_t = module->addWire(NEW_ID2_SUFFIX("tmp"), sig_a.size() / 2);
	...
	if (cell->type == ID($reduce_xnor)) {
		RTLIL::SigSpec sig_t = module->addWire(NEW_ID2_SUFFIX("tmp"));
		RTLIL::Cell *gate = module->addCell(NEW_ID2, ID($_NOT_));
		transfer_src(gate, cell);
...
static void logic_reduce(RTLIL::Module *module, RTLIL::SigSpec &sig, RTLIL::Cell *cell)
{
	while (sig.size() > 1)
	{
		RTLIL::SigSpec sig_t = module->addWire(NEW_ID2_SUFFIX("tmp"), sig.size() / 2);
		...
		for (int i = 0; i < sig.size(); i += 2)
		{
			...
			RTLIL::Cell *gate = module->addCell(NEW_ID2, ID($_OR_));
			transfer_src(gate, cell);
...
void simplemap_eqne(RTLIL::Module *module, RTLIL::Cell *cell)
{
	...
	RTLIL::SigSpec xor_out = module->addWire(NEW_ID2_SUFFIX("tmp"), max(GetSize(sig_a), GetSize(sig_b)));
	RTLIL::Cell *xor_cell = module->addXor(NEW_ID2_SUFFIX("xor"), sig_a, sig_b, xor_out, is_signed);
	transfer_src(xor_cell, cell);
	simplemap_bitop(module, xor_cell);
	module->remove(xor_cell);

	RTLIL::SigSpec reduce_out = is_ne ? sig_y : module->addWire(NEW_ID2_SUFFIX("tmp"));
	RTLIL::Cell *reduce_cell = module->addReduceOr(NEW_ID2_SUFFIX("reduce_or"), xor_out, reduce_out);
	transfer_src(reduce_cell, cell);
	simplemap_reduce(module, reduce_cell);
	module->remove(reduce_cell);

	if (!is_ne) {
		RTLIL::Cell *not_cell = module->addLogicNot(NEW_ID2_SUFFIX("not"), reduce_out, sig_y);
...
void simplemap_mux(RTLIL::Module *module, RTLIL::Cell *cell)
{
	...
	for (int i = 0; i < GetSize(sig_y); i++) {
		RTLIL::Cell *gate = module->addCell(NEW_ID2, ID($_MUX_));
		transfer_src(gate, cell);
		gate->setPort(ID::A, sig_a[i]);
		gate->setPort(ID::B, sig_b[i]);
...
void simplemap_tribuf(RTLIL::Module *module, RTLIL::Cell *cell)
{
	...
	for (int i = 0; i < GetSize(sig_y); i++) {
		RTLIL::Cell *gate = module->addCell(NEW_ID2, ID($_TBUF_));
		transfer_src(gate, cell);
		gate->setPort(ID::A, sig_a[i]);
...
void simplemap_bmux(RTLIL::Module *module, RTLIL::Cell *cell)
{
	...
	for (int idx = 0; idx < GetSize(sel); idx++) {
		SigSpec new_data = module->addWire(NEW_ID2_SUFFIX("tmp"), GetSize(data)/2);
...
void simplemap_lut(RTLIL::Module *module, RTLIL::Cell *cell)
{
	...
	for (int idx = 0; GetSize(lut_data) > 1; idx++) {
		SigSpec new_lut_data = module->addWire(NEW_ID2_SUFFIX("tmp"), GetSize(lut_data)/2);
...
void simplemap_sop(RTLIL::Module *module, RTLIL::Cell *cell)
{
	...
	for (int i = 0; i < depth; i++) {
		...
		products.append(GetSize(in) > 0 ? module->Eq(NEW_ID2_SUFFIX("eq"), in, pat) : State::S1);
	}

	module->connect(cell->getPort(ID::Y), module->ReduceOr(NEW_ID2_SUFFIX("reduce_or"), products));
}
