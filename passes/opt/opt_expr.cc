		...
		if (cell->type.in(ID($xor), ID($xnor)) && i == GRP_CONST_A) {
			...
			for (int i = 0; i < GetSize(new_y); i++) {
				...
				if (!keepdc && (undef || new_a[i] == new_b[i])) {
					...
					undef_a.append(new_a[i]);
					if (cell->type == ID($xor))
						undef_b.append(new_a[i] == State::S1 ? module->Not(NEW_ID2_SUFFIX("grp_undef_b"), new_b[i]).as_bit() : new_b[i]);
					else if (cell->type == ID($xnor))
						undef_b.append(new_a[i] == State::S1 ? new_b[i] : module->Not(NEW_ID2_SUFFIX("grp_undef_b"), new_b[i]).as_bit());
					else log_abort();
					undef_y.append(new_y[i]);
				}
				else {
...
#define ACTION_DO_Y(_v_) ACTION_DO(ID::Y, RTLIL::SigSpec(RTLIL::State::S ## _v_))
		...
		if (cell->type.in(ID($_XOR_), ID($_XNOR_)) || (cell->type.in(ID($xor), ID($xnor)) && GetSize(cell->getPort(ID::A)) == 1 && GetSize(cell->getPort(ID::B)) == 1 && !cell->getParam(ID::A_SIGNED).as_bool()))
		{
			...
			if (sig_b == State::S0 || sig_b == State::S1) {
				if (cell->type.in(ID($xor), ID($_XOR_))) {
					SigSpec sig_y;
					if (cell->type == ID($xor))
						sig_y = (sig_b == State::S1 ? module->Not(NEW_ID2_SUFFIX("xor_inv"), sig_a).as_bit() : sig_a);
					else if (cell->type == ID($_XOR_))
						sig_y = (sig_b == State::S1 ? module->NotGate(NEW_ID2_SUFFIX("xor_inv"), sig_a) : sig_a);
					else log_abort();
				...
				if (cell->type.in(ID($xnor), ID($_XNOR_))) {
					...
					if (cell->type == ID($xnor)) {
						sig_y = (sig_b == State::S1 ? sig_a : module->Not(NEW_ID2_SUFFIX("xnor_inv"), sig_a).as_bit());
						int width = cell->getParam(ID::Y_WIDTH).as_int();
						sig_y.append(RTLIL::Const(State::S1, width-1));
					}
					else if (cell->type == ID($_XNOR_))
						sig_y = (sig_b == State::S1 ? sig_a : module->NotGate(NEW_ID2_SUFFIX("xnor_inv"), sig_a));
					else log_abort();
		...
		if (cell->type.in(ID($and), ID($or), ID($xor), ID($xnor)))
		{
			...
			if (a_fully_const != b_fully_const)
			{
				...
				if (cell->type == ID($and)) {
					...
					if (!y_group_x.empty()) {
						if (keepdc)
							y_new_x = module->And(NEW_ID2_SUFFIX("and"), Const(State::Sx, GetSize(y_group_x)), b_group_x);
						else
					...
					if (!y_group_x.empty()) {
						if (keepdc)
							y_new_x = module->Or(NEW_ID2_SUFFIX("or"), Const(State::Sx, GetSize(y_group_x)), b_group_x);
						else
							y_new_x = Const(State::S1, GetSize(y_group_x));
					}
				} else if (cell->type.in(ID($xor), ID($xnor))) {
					if (!y_group_0.empty()) y_new_0 = b_group_0;
					if (!y_group_1.empty()) y_new_1 = module->Not(NEW_ID2_SUFFIX("inv"), b_group_1);
					if (!y_group_x.empty()) {
						if (keepdc)
							y_new_x = module->Xor(NEW_ID2_SUFFIX("xor"), Const(State::Sx, GetSize(y_group_x)), b_group_x);
		...
		if (cell->type == ID($bwmux))
		{
			...
			else if (sig_a.is_fully_def() || sig_b.is_fully_def())
			{
				...
				if (flip) {
					if (!y_group_0.empty()) y_new_0 = module->And(NEW_ID2_SUFFIX("and"), b_group_0, module->Not(NEW_ID2_SUFFIX("inv"), s_group_0));
					if (!y_group_1.empty()) y_new_1 = module->Or(NEW_ID2_SUFFIX("or"), b_group_1, s_group_1);
				} else {
					if (!y_group_0.empty()) y_new_0 = module->And(NEW_ID2_SUFFIX("and"), b_group_0, s_group_0);
					if (!y_group_1.empty()) y_new_1 = module->Or(NEW_ID2_SUFFIX("or"), b_group_1, module->Not(NEW_ID2_SUFFIX("inv"), s_group_1));
				}
			...
			if (cell->type == ID($alu))
			{
				...
				for (i = 0; i < minsz; i++) {
					...
					if (b == ((bi ^ ci) ? State::S1 : State::S0)) {
						module->connect(sig_y[i], a);
						module->connect(sig_x[i], ci ? module->Not(NEW_ID2_SUFFIX("x_inv"), a).as_bit() : a);
						module->connect(sig_co[i], ci ? State::S1 : State::S0);
					}
					else if (a == (ci ? State::S1 : State::S0)) {
						module->connect(sig_y[i], bi ? module->Not(NEW_ID2_SUFFIX("y_inv"), b).as_bit() : b);
						module->connect(sig_x[i], (bi ^ ci) ? module->Not(NEW_ID2_SUFFIX("x_inv"), b).as_bit() : b);
						module->connect(sig_co[i], ci ? State::S1 : State::S0);
					}
					else
						break;
			...
			if (identity_wrt_a || identity_wrt_b)
			{
				...
				if (cell->type == ID($alu)) {
					...
					if (sig_ci == State::S1) {
						...
						module->connect(cell->getPort(ID::X), module->Not(NEW_ID2_SUFFIX("ci_inv"), a));
		...
		if (cell->type == ID($pow) && cell->getPort(ID::A).is_fully_const() && !cell->parameters[ID::B_SIGNED].as_bool()) {
			...
			if (onehot && (!cell->parameters[ID::A_SIGNED].as_bool() || bit_idx < sig_a.size() - 1)) {
				...
				if (bit_idx == 1) {
					...
					SigSpec y_wire = module->addWire(NEW_ID2_SUFFIX("shl_y"), y_size);
					cell->setPort(ID::Y, y_wire);

					module->addShl(NEW_ID2_SUFFIX("shl"), Const(State::S1, 1), y_wire, sig_y);
				}
				did_something = true;
				goto next_cell;
			}
				...
				if (!keepdc && sig_b.is_onehot(&exp) && !(b_signed && exp == GetSize(sig_b) - 1))
				{
					if (cell->type.in(ID($div), ID($divfloor)))
					{
						...
						if (is_truncating && a_signed && GetSize(sig_a) != 0 && exp != 0) {
							Wire *flooring = module->addWire(NEW_ID2_SUFFIX("flooring"), sig_y.size());
							cell->setPort(ID::Y, flooring);

							SigSpec a_sign = sig_a[sig_a.size()-1];
							SigSpec rem_nonzero = module->ReduceOr(NEW_ID2_SUFFIX("rem_nonzero"), sig_a.extract(0, exp));
							SigSpec should_add = module->And(NEW_ID2_SUFFIX("should_add"), a_sign, rem_nonzero);
							module->addAdd(NEW_ID2_SUFFIX("add"), flooring, should_add, sig_y);
						}
					...
					else if (cell->type.in(ID($mod), ID($modfloor)))
					{
						...
						if (is_truncating && a_signed && GetSize(sig_a) != 0 && exp != 0)
						{
							...
							SigSpec a_sign = sig_a[sig_a.size()-1];
							SigSpec rem_nonzero = module->ReduceOr(NEW_ID2_SUFFIX("rem_nonzero"), sig_a.extract(0, exp));
							SigSpec extend_bit = module->And(NEW_ID2_SUFFIX("extend_bit"), a_sign, rem_nonzero);

							truncating.append(extend_bit);
							module->addPos(NEW_ID2_SUFFIX("pos"), truncating, sig_y, true);
						}
						else
						{
			...
			if (const_sig.is_fully_def() && const_sig.is_fully_const())
			{
				...
				{ /* unsigned */
					...
					if (const_sig.is_onehot(&const_bit_hot) && const_bit_hot < var_width)
					{
						...
						if (cmp_type == ID($lt))
						{
							...
							module->addLogicNot(NEW_ID2_SUFFIX("not"), var_high_sig, cell->getPort(ID::Y));
							remove = true;
						}
						if (cmp_type == ID($ge))
						{
							condition   = stringf("unsigned X>=%s", log_signal(const_sig));
							replacement = stringf("|X[%d:%d]", var_width - 1, const_bit_hot);
							module->addReduceOr(NEW_ID2_SUFFIX("reduce_or"), var_high_sig, cell->getPort(ID::Y));
							remove = true;
						}
					}
				...
				{ /* signed */
					...
					if (const_sig.is_fully_zero() && cmp_type == ID($ge))
					{
						condition   = "signed X>=0";
						replacement = stringf("X[%d]", var_width - 1);
						module->addLogicNot(NEW_ID2_SUFFIX("not"), var_sig[var_width - 1], cell->getPort(ID::Y));
						remove = true;
					}
				}
