...
void merge_operators(RTLIL::Module *module, RTLIL::Cell *mux, const std::vector<OpMuxConn> &ports, const ExtSigSpec &operand, const SigMap &sigmap)
{
	...
	for (auto &operand : muxed_operands) {
		...
		if (operand.sign != muxed_operands[0].sign)
			operand = ExtSigSpec(module->Neg(NEW_ID2_SUFFIX("neg"), operand.sig, operand.is_signed));
	}
	...
	Wire *new_out = module->addWire(NEW_ID2_SUFFIX("new_out"), conn_op_offset + conn_width);
	...
	if (GetSize(shared_pmux_s) == 1) {
		mux_to_oper = module->Mux(NEW_ID2_SUFFIX("mux"), shared_pmux_a, shared_pmux_b, shared_pmux_s);
	} else {
		mux_to_oper = module->Pmux(NEW_ID2_SUFFIX("pmux"), shared_pmux_a, shared_pmux_b, shared_pmux_s);
	}

	if (shared_op->type.in(ID($alu))) {
		shared_op->setPort(ID::X, module->addWire(NEW_ID2_SUFFIX("X"), GetSize(new_out)));
		shared_op->setPort(ID::CO, module->addWire(NEW_ID2_SUFFIX("CO"), GetSize(new_out)));
	}
