...
struct OptMemFeedbackWorker
{
	...
	RTLIL::SigBit conditions_to_logic(pool<dict<RTLIL::SigBit, bool>> &conditions, SigBit olden, Mem &mem)
	{
		...
		for (auto &cond : conditions) {
			...
			terms.append(module->Ne(NEW_MEM_ID_SUFFIX("cond_ne"), sig1, sig2));
		}
		...
		if (GetSize(terms) > 1)
			terms = module->ReduceAnd(NEW_MEM_ID_SUFFIX("cond_reduce_and"), terms);
	...
	void translate_rd_feedback_to_en(Mem &mem)
	{
		...
		for (auto &it : portbit_conds)
		{
			...
			port.en[bit] = conditions_to_logic(it.second, port.en[bit], mem);
